...
export function adminDb() {
  ...
  const missing = [!url && "NEXT_PUBLIC_SUPABASE_URL", !secretKey && "SUPABASE_SECRET_KEY"].filter(Boolean);
  if (missing.length) {
    // Variable names only, so a misconfigured environment is obvious from the screen.
    throw new HttpError(500, `Server is missing ${missing.join(" and ")}.`);
  }
