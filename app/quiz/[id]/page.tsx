...
export default function QuizPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create answers
      const answerTransactions = Object.entries(answers).map(([questionId, value]) => {
        const answerId = genId();
        return tx.answers[answerId].update({
          responseId,
          questionId,
          value,
        });
      ...
      await db.transact(answerTransactions);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <Card className="max-w-md w-full text-center">
          ...
          <p className="text-[var(--muted)] mb-6">
            This survey may have been removed or is not available.
          </p>
          <Link href="/">
            <Button>Back to Home</Button>
          </Link>
